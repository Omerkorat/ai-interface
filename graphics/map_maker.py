...
#Up here since constants use this function
def rgb2hex(rgb_tuple):
    '''Converts an 0.0 to 1.0 rgb tuple (R,G,B) to hexa'''
    rescale=(int(rgb_tuple[0]*255), int(rgb_tuple[1]*255), int(rgb_tuple[2]*255))
    return ('#%02x%02x%02x' % rescale)

# Constants:

BACKGROUND_COLOR = rgb2hex((0.80,0.80,0.90))
WALL_WIDTH = 3
DOOR_LENGTH = 20
HEIGHT, WIDTH = 2000, 1200 # Canvas dimensions
CANVAS_MID_X = WIDTH / 2
CANVAS_MID_Y = HEIGHT / 2

defWallColor=rgb2hex((0,0,0))
defFloorColor=rgb2hex((0.6,0.6,0.6))
defDoorColor=rgb2hex((0.02,0.0,0.65))
defLockedDoorColor=rgb2hex((0.64,0.12,0.12))
defUnlockedDoorColor=rgb2hex((0.012,0.98,0.04))

TestColor=defWallColor
...
def temp2color(T):
    ...
    Tmid=25.0
    if T<=Tmid:
        return rgb2hex((0,0,(Tmid-T)/(Tmid-Tmin)));
    if T>Tmid:
        return rgb2hex(((T-Tmid)/(Tmax-Tmid),0,0));
...
def main():
    ...
    map.draw_polygon_room("engine-room", [(50, 50), (50, 120), (80, 160), (120, 120), (120, 50)], defWallColor, defFloorColor,
                            [
                                [0, 10, defDoorColor],
                                [1, 10, defLockedDoorColor],
                                [2, 10, defLockedDoorColor],
                                [3, -40, defUnlockedDoorColor],
                                [4, -40, defUnlockedDoorColor],
                            ])
...
#     map_w.close_door("engine-room", 3)
    ...
    map.change_wall_color("engine-room", 2, TestColor)
